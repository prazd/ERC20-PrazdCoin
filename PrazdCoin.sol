...
// ----------------------------------------------------------------------------
contract PrazdCoin is ERC20Interface, Owned {
    using SafeMath for uint;

    string public symbol = 'PCN';
    string public  name = 'PrazdCoin';
    uint8 public decimals = 18;
    uint public _totalSupply = 10000000 * 10*uint(decimals);
 ...
 //       balances[owner] = _totalSupply;
  //      Transfer(address(0), owner, _totalSupply);
   // }
    ...
    assert(_totalSupply + _value >= _totalSupply && balances[_to] + _value >= balances[_to]); // Проверяем на переполнение переменных
    balances[_to] += _value;                                                                // Прибавляем токены к аккаунту  
    _totalSupply += _value;                                                                  // Прибавляем это же кол-во к общему предложению
      }
